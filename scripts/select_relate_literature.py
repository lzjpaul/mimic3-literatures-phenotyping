...
class Doc2WordList(object):
    def __init__(self, doc_path, stop_word=[]):
        ...
        for sentence in sentences_list:
            for keywords in entity_key_list:
                if NLP_Utility.keyword_in_sentence(keyword=keywords.split(" "), sentence=sentence, percent=1):
                    self.entity_count[keywords+"@"+entity_map[keywords]]=\
                        self.entity_count[keywords+"@"+entity_map[keywords]]+1 \
                            if keywords+"@"+entity_map[keywords] in self.entity_count else 1
...
if __name__ == '__main__':
    ...
    for file_path in file_path_list:
        doc2word_list = Doc2WordList(doc_path=file_path, stop_word=[])
